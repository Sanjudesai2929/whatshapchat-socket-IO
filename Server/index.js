...
io.on("connection", async (client) => {
    ...
    client.on('connected-user', async (data) => {
        ...
        const connectMsg = await Message.find({$or :[{$and:[{targetId: data.connected_user,sentBy:data.current_user}]},{$and:[{targetId: data.current_user,sentBy:data.connected_user}]} ] }).sort({time:1}).sort({date:-1})
